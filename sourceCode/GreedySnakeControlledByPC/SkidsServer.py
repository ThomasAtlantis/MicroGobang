...
import screen, network, usocket, text
...
if __name__ == '__main__':
    ...
    try:
        ...
        ip = wlan.ifconfig()[0]  # 获取自己的IP

        print(ip)
         
        listenSocket = usocket.socket(usocket.AF_INET, usocket.SOCK_STREAM)
        listenSocket.bind((ip, PORT))  # 绑定socket
        listenSocket.settimeout(None)  # 超时时间无限大
        listenSocket.setblocking(True) # 阻塞模式
        listenSocket.listen(1) # 允许最大连接数为1，防止多方控制

        while True:
            conn, addr = listenSocket.accept()  # 阻塞，等待连接
            print(addr, "connected")
            while True:
                data = conn.recv(512)  # 阻塞，等待消息
                data_str = data.decode('utf-8')

                if "R" in data_str:  # 用in而不是==，是因为客户端消息发送过快可能导致多个消息连在一起，产生若干个R的字符串
                    snakeGame.key_release(0)  # 对应贪吃蛇的转向逻辑
                    print("R")
                elif "D" in data_str:
                    snakeGame.key_release(1)
                    print("D")
                elif "L" in data_str:
                    snakeGame.key_release(2)
                    print("L")
                elif "U" in data_str:
                    snakeGame.key_release(3)
                    print("U")
                if snakeGame.gameover == True:  # 如果游戏结束则重新开始
                    snakeGame.initial()
                else:
                    snakeGame.move()
